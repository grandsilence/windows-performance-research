...
![Меню навигации](research/menu.gif)
...
* в `1803` появились заплатки Spectre и Meltdown, в `1709` их ещё не интегрировали, поэтому отключать заплатки утилитой [Inspectre](https://www.grc.com/inspectre.htm) для `1709` не нужно.
...
9. Установлен режим энергопотребления "Максимальная производительность", отключена заставка, выключено отключение экрана и USB портов, выключены таймеры пробуждения.
...
### Способ установки Windows 10 21H1 (1055)

1. Установить Windows **без интернета** используя [оригинальный образ Version 21H1 (Updated June 2021)](https://nnmclub.to/forum/viewtopic.php?t=1479018).
2. Заранее [скачать обновление KB5004476 (прямая ссылка)](http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/06/windows10.0-kb5004476-x64_d921e030a0749cb41551bbfa9b53c35edbb1e763.msu).
3. Распаковать файл `Windows10.0-KB5004476-x64.cab` в папку `C:\update`
4. Открыть командную строку с правами администратора и запустить `DISM`:  

    ```bat
    dism /online /add-package /packagepath:"C:\update\Windows10.0-KB5004476-x64.cab
    ```

5. Дождаться установки, может показаться что подвис.
6. Запустить Win10Tweaker и выбрать нужные твики, главное **после удаления мусорных приложений отключить службы обновлений**.
7. Включить интернет.
