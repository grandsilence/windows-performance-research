...
* после `1809` и выше (да, именно 1809), внесли изменения в системный таймер изменив его частоту с 3 MHz до 10 MHz, повысив точность (близка к HPET) в угоду отзывчивости системы.  
  **Это ключевой момент который** может оказывать влияние на отзывчивость системы, особенно на старых процессорах вроде `i5 2500`. Для свежих процессоров выше 8 поколения – не так критично.
